...
async def ask_question(request: AskQuestionRequest) -> Any:
...
async def get_analyst_report(request: ReportGenerationRequest) -> Any:
    if not request.indicator.strip():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Indicator cannot be empty.")
...
async def get_executive_summary(request: ReportGenerationRequest) -> Any:
    if not request.indicator.strip():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Indicator cannot be empty.")
